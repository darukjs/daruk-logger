...
class Logger {
  constructor (options = {}) {
    ...
    const format = typeof options.format === 'string' ? winston.format[options.format]() : options.format(options)
  ...
  log (level, msgs) {
    ...
    if (this.options.fileInfo || this.levels[level] === 0) {
      ...
      fileInfo = runTimeTrace.replace(process.cwd(), '').replace(/(^\s+|\s+$)/, '').replace(/(^.*?\()/, '').replace(/(\)$)/, '')
      // 将自定义的 fileInfo 作为最终文件信息的前缀
      fileInfo = this.customFileInformation ? `[${this.customFileInformation}] ${fileInfo}` : fileInfo
    }
    let resMsg
    ...
    const logExt = typeof optionLogExt === 'function' ? optionLogExt() : optionLogExt
